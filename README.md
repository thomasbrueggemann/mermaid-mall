...
next to it are only needed for installing it as an app.
...
what keeps the whole thing a single self-contained offline bundle.
...
`mission`) plus `cheat(n)`, `walkToTarget()` and `teleportToTarget()` — handy
for testing, and for a grown-up who wants to skip ahead.
